...
/**
 * super-admin-resend-trainers.js
 *
 * Endpoint   : POST /dietitian/api/web/super-admin-resend-trainers
 * Auth       : Bearer JWT (authMiddleware must run before this handler)
 * Authorized : admin | super_admin (trainer is explicitly denied)
 *
 * VAPT HTML Injection remediation:
 *  - Invitation names are read from DB and treated as untrusted/legacy data.
 *  - Every plain-text value sent into the Resend HTML template is escaped.
 *  - The server-generated invitation URL is kept as a URL value.
 */
...
async function writeAuthLogSafe(
  ...
  try {
    ...
    console.error(
      "SUPER_ADMIN_RESEND_TRAINERS_AUDIT_FAILED:",
      error?.code ||
        error?.message
    );
  }
...
// ─── Invite resolution + permission ─────────────────────────────────────────

async function getInviteForUpdate(
  conn,
  inviteId
) {
  const [rows] =
...
async function actorCanManageInvite(
  conn,
  actor,
  actorEmail,
  invite
  ...
  /*
  |--------------------------------------------------------------------------
  | Trainer must never resend invitations
  |--------------------------------------------------------------------------
  */
  ...
  /*
  |--------------------------------------------------------------------------
  | Actor directly created / owns invitation
  |--------------------------------------------------------------------------
  */
  ...
  /*
  |--------------------------------------------------------------------------
  | Super admin can manage invites belonging to their active admins
  |--------------------------------------------------------------------------
  */
...
const superAdminResendTrainers =
  async (req, res) => {
    /*
    |--------------------------------------------------------------------------
    | Prevent caching
      ...
      if (
        ...
        await writeAuthLogSafe(
          req,
          {
            ...
            failureReason:
              "Actor not allowed to resend this invitation",
          }
        );
        ...
        return res
          ...
          .json({
            ...
            message:
              "You are not allowed to resend this invitation",
          });
      }
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Revoked invitation cannot be resent. Create a new invitation.",
          });
      }
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Only pending, sent, or expired invitations can be resent",
          });
      }
...
*/
      ...
      */
      ...
      |
      | Even though new invitations now reject HTML during creation, OLD rows
      | can still contain:
      |
      ...
      | {{7*7}}
      |
      | Therefore stored values are treated as untrusted and escaped again
      | immediately before entering the external HTML email template.
      |--------------------------------------------------------------------------
      */
      ...
      const emailResult =
        await sendResendTemplateEmail(
...

          {
            ...
            |--------------------------------------------------------------------------
            |
            | The invite link is generated by the server using randomBytes()
            | and the configured frontend URL. It is not a human-name value.
            |--------------------------------------------------------------------------
            */
      ...
      */
      ...
      if (
        ...
        console.error(
          "SUPER_ADMIN_RESEND_TRAINERS_EMAIL_FAILED:",
          {
            invitation_id:
              inviteId,
        ...
        return res
          ...
          .json({
            ...
            message:
              "Invitation email could not be resent",
      ...
      |--------------------------------------------------------------------------
      |
      | Guarded against concurrent acceptance.
      |--------------------------------------------------------------------------
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | Concurrent acceptance / state change
      |--------------------------------------------------------------------------
      */
      ...
      if (
        ...
        await writeAuthLogSafe(
          req,
          {
            ...
            failureReason:
              "Invitation email was sent, but invite status could not be updated",
          }
        );
      ...
      */
      ...
      console.error(
        "SUPER_ADMIN_RESEND_TRAINERS_ERROR:",
        {
          code:
            error?.code,
...
//  * super-admin-resend-trainers.js
//  *
//  * Converted from: super-admin-resend-trainers.php (+ user-invite-action-common.php helpers)
...
//  *    `super_admin_invite_resent` / `super_admin_invite_resend_failed`;
//  *    everyone else emits `user_invite_resent` / `user_invite_resend_failed`.
//  *  - Response shape matches the PHP: { ok, message, data, meta } where meta
//  *    carries { resent_by, resent_by_role }.
//  *
//  * rsui_actor_can_manage_invite() parity:
...
// // Only these invite states may be resent (PHP whitelist).
// const RESENDABLE_STATUSES = new Set(["pending", "sent", "expired"]);
...
//     );
//   } catch (err) {
//     console.error("SUPER_ADMIN_RESEND_TRAINERS_AUDIT_FAILED:", err?.code || err?.message);
//   }
// }
...
// /**
//  * Reimplementation of rsui_actor_can_manage_invite() (PHP). Keeps the existing
...
// /**
//  * POST /dietitian/api/web/super-admin-resend-trainers
//  *
...
//  *   }
//  */
// const superAdminResendTrainers = async (req, res) => {
...
//       await writeAuthLogSafe(req, {
//         eventType: eventFor("resend_denied"),
//         userId: actorEmail,
//         role: actorRole,
...
//     // Only pending / sent / expired invites may be resent (PHP whitelist).
//     if (!RESENDABLE_STATUSES.has(status)) {
//       await conn.rollback();
//       conn.release();
//       conn = null;
//       return res.status(409).json({
//         ok: false,
//         message: "Only pending, sent, or expired invitations can be resent",
//       });
//     }
...
//     if (!emailResult.ok) {
//       console.error("SUPER_ADMIN_RESEND_TRAINERS_EMAIL_FAILED:", {
//         invitation_id: inviteId,
//         status: emailResult.status,
//         error: APP_DEBUG ? emailResult.error : undefined,
//       });
//       await writeAuthLogSafe(req, {
//         eventType: eventFor("resend_failed"),
//         userId: actorEmail,
//         role: actorRole,
//         partnerCode: actorCode,
...
//         identifier: invitedEmail,
//         success: false,
//         failureReason: "Invitation email was sent, but invite status could not be updated",
//       });
//       return res.status(409).json({
//         ok: false,
//         message: "Invitation email was sent, but invite status could not be updated",
//       });
//     }

//     // ── 6. Audit success ────────────────────────────────────────────────────
//     await writeAuthLogSafe(req, {
//       eventType: eventFor("resent"),
//       userId: actorEmail,
//       role: actorRole,
//       partnerCode: actorCode,
//       identifier: invitedEmail,
...
//     // ── 7. Respond (matches the PHP JSON shape, incl. meta) ─────────────────
//     const updatedInvite = {
//       ...invite,
//       status: "pending",
...
//       meta: {
//         resent_by: actorEmail,
//         resent_by_role: actorRole,
//       },
//     };
...
//     console.error("SUPER_ADMIN_RESEND_TRAINERS_ERROR:", {
//       code: err?.code,
//       errno: err?.errno,
...
//     await writeAuthLogSafe(req, {
//       eventType: eventFor("resend_error"),
...
// module.exports = { superAdminResendTrainers };