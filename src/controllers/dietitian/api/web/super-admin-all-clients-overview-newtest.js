...
/**
 * super-admin-all-clients-overview-newtest.js
 *
 * Cloned from: super-admin-all-clients-overview.js — identical logic, but the
 ...
 *
 * Endpoint   : POST /dietitian/api/web/super-admin-all-clients-overview-newtest
...
async function writeAuthLogSafe(req, {
  ...
  try {
    ...
    console.error("SUPER_ADMIN_ALL_CLIENTS_NEWTEST_AUDIT_FAILED:", err?.code || err?.message);
  }
}
...
async function buildDietPlanSql() {
  ...
  try {
    ...
    console.error("DIET_PLAN_NEWTEST_COLUMN_LOOKUP_FAILED:", err?.code || err?.message);
    return fallback;
  }
...
/**
 * POST /dietitian/api/web/super-admin-all-clients-overview-newtest
 *
 ...
 *   }
 */
const superAdminAllClientsOverviewNewtest = async (req, res) => {
  ...
  try {
    ...
    console.error("SUPER_ADMIN_ALL_CLIENTS_NEWTEST_ERROR:", {
      code: err?.code,
      errno: err?.errno,
...
module.exports = { superAdminAllClientsOverviewNewtest };
