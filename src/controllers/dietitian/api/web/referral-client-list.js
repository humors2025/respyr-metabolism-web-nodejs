 ...
 *  - partner_code allowlist validation.
 *  - Search length/type/character validation.
 *  - Strict page/limit numeric validation.
 *  - Every query is fully parameterized.
 *  - Cache-Control: no-store.
 *
 * HIPAA controls:
 *  - Minimum-necessary columns.
 *  - PHI in audit logs is HMAC-SHA256 hashed.
 *  - Every list view is audited.
...
/**
 * Validate an optional partner/trainer code supplied by the client.
 */
function validatePartnerCodeInput(value) {
  if (
    value === null ||
    value === undefined ||
    value === ""
...
/**
 * Search is performed in memory, not interpolated into SQL.
 * Still reject HTML / JS / template-like input.
 */
function validateSearchInput(value) {
  ...
  if (
    ...
    return {
      ok: false,
      message:
        "Search must be maximum 100 characters",
    };
  }
...
async function writeAuthLogSafe(
  ...
  try {
    ...
    console.error(
      "REFERRAL_CLIENT_LIST_AUDIT_FAILED:",
      err?.code ||
        err?.message
    );
  }
...
async function fetchSubscriptionRows(
  ...
  const subscriptionCols =
    await tableColumns(
      "trainer_client_plan_subscriptions"
    );
    ...
    await pool.execute(
      `
        SELECT
          ...
          tc.profile_id
            AS mapped_accepted_profile_id,

          tc.profile_name
            AS accepted_client_name,

          tc.phone_no
            AS accepted_client_phone,

          tc.email
            AS accepted_client_email,

          tc.dietician_id
            AS accepted_client_dietician_id,

          tc.dttm
            AS accepted_client_joined_at,
...
async function fetchInviteRows(
  ...
  const subscriptionRedeemSelect =
    subscriptionCols
      .redeem_code
      ? `COALESCE(
           ps_direct.redeem_code,
           ps_latest.redeem_code
         ) AS subscription_redeem_code`
...
async function getLatestClientSubscriptionByProfile(
  profileId
) {
  const pid =
    cleanValue(
  ...
  const [rows] =
    await pool.execute(
      `
        SELECT
          id
            AS client_subscription_id,

          dietician_id
            AS client_subscription_dietician_id,

          profile_id
            AS client_subscription_profile_id,

          plan_code
            AS client_subscription_plan_code,

          plan_name
            AS client_subscription_plan_name,

          status
            AS client_subscription_status,

          subscription_start_date
            AS client_subscription_start_date,

          subscription_end_date
            AS client_subscription_end_date,

          current_period_end
            AS client_subscription_current_period_end,

          created_at
            AS client_subscription_created_at,

          updated_at
            AS client_subscription_updated_at
...
async function fetchLegacyClientRows(
  codes,
  knownInviteKeys
) {
  if (
  ...
  const selectParts = [
    profileIdCol
      ? `\`${profileIdCol}\` AS profile_id`
      : "NULL AS profile_id",

    nameCol
      ? `\`${nameCol}\` AS client_name`
      : "NULL AS client_name",

    phoneCol
      ? `\`${phoneCol}\` AS client_mobile`
      : "NULL AS client_mobile",

    emailCol
      ? `\`${emailCol}\` AS client_email`
      : "NULL AS client_email",

    dateCol
      ? `\`${dateCol}\` AS created_on`
      : "NULL AS created_on",
  ...
  for (
    ...
    const subscriptionRow =
      await getLatestClientSubscriptionByProfile(
        profileId
      );
...
function acceptedClientFromRow(
  row
) {
  ...
  return {
    ...
    name:
      row
        .accepted_client_name ??
      null,

    phone:
      row
        .accepted_client_phone ??
      null,

    email:
      row
        .accepted_client_email !==
        undefined &&
      row
        .accepted_client_email !==
        null
    ...
    dietician_id:
      row
        .accepted_client_dietician_id ??
      null,

    joined_at:
      toMysqlDateTime(
        row
          .accepted_client_joined_at
      ),
...
function freeTrialSubscriptionInfo(
  row
) {
  ...
  if (
    ...
    return {
      ...
      extend_to_total_days:
        14,

      message:
        "No latest client_subscriptions row found",
    };
  }
  ...
  const startRaw =
    row
      .client_subscription_start_date ??
    null;

  const endRaw =
    row
      .client_subscription_end_date ??
    null;

  const cpeRaw =
    row
      .client_subscription_current_period_end ??
    null;
  ...
  if (
    ...
    message =
      "extend14days available";
  } else if (
    !isFreeTrial
  ) {
    message =
      "Latest subscription is not FREE_TRIAL";
  } else if (
    !notCancelled
  ) {
    message =
      "Cancelled subscription cannot be extended";
  } else if (
    trialTotalDays !==
      null &&
    trialTotalDays >
    ...
    message =
      "Already extended or trial duration is more than 7 days";
  } else {
    message =
      "extend14days not available";
  }
...
function buildKnownKeysFromInvites(
  inviteRows,
  subscriptionRows
) {
  const keys =
...
function applyStrictScopeFilter(
  rows,
  viewCodes
) {
  return rows.filter(
    (row) =>
      rowPassesStrictScope(
        row,
        viewCodes
      )
  );
...
const referralClientList =
  async (
    ...
    */
      ...
      if (
        ...
        return res
          ...
          .json({
            ...
            message:
              "Valid actor_user_id is required",
          });
      }
    ...
    /*
    |--------------------------------------------------------------------------
    | partner_code validation
    |--------------------------------------------------------------------------
    */

    const selectedCodeResult =
      validatePartnerCodeInput(
        body.partner_code
      );

    if (
      !selectedCodeResult.ok
    ) {
      return res
        .status(422)
        .json({
    ...
    */
    ...
    try {
      ...
      */
      ...
      if (
        ...
        await writeAuthLogSafe(
          req,
          {
            ...
            failureReason:
              "partner_code not allowed for actor",
          }
        );
        ...
        return res
          ...
          .json({
            ...
            message:
              "You are not allowed to view this partner code",
          });
      }
      ...
      /*
      |--------------------------------------------------------------------------
      | Fetch subscription and invite rows
      |--------------------------------------------------------------------------
      */

      const subscriptionRowsRaw =
        await fetchSubscriptionRows(
          viewCodes
        );

      const inviteRowsRaw =
        await fetchInviteRows(
          viewCodes
        );
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | Deduplicate invite rows represented by subscriptions
      |--------------------------------------------------------------------------
      */

      const subscriptionInviteIds =
        new Set();
      ...
      */
      ...
      const knownInviteKeys =
        buildKnownKeysFromInvites(
          inviteRowsRaw,
          subscriptionRowsRaw
        );

      const legacyRows =
        await fetchLegacyClientRows(
          viewCodes,
          knownInviteKeys
        );
      ...
      */
      ...
      return res
        ...
        .json({
          ...
          message:
            "Referral clients fetched successfully",
