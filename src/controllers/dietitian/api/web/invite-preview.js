 ...
 * Auth       : NONE (public)
 *
 * The single-use invite token is the credential used to fetch the
 * invitation information required by the signup screen.
 *
 * HTML Injection hardening:
 *  - Legacy invitation rows are treated as untrusted.
 *  - first_name / last_name are validated again before being returned.
 *  - Malicious legacy values are never returned to the signup frontend.
 */
    ...
    */
    ...
    try {
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | HTML Injection / legacy stored-data protection
      |--------------------------------------------------------------------------
      |
      | New invite endpoints already reject malicious names during creation.
      |
      | But old rows may still contain:
      |
      | <h1>HTML INJECTION</h1>
      | <script>alert(1)</script>
      | {{7*7}}
      |
      | Do not return those values to the public signup frontend.
      |--------------------------------------------------------------------------
      */
      ...
      if (
        ...
        await writeAuthLogSafe(
          req,
          {
            ...
            failureReason:
              !firstNameResult.ok
                ? "invalid_stored_first_name"
                : "invalid_stored_last_name",
          }
        );
        ...
        return sendJson(
          ...
          409,
          {
            ...
            message:
              "Invitation contains invalid name data. Please request a new invitation.",
          }
        );
      }
