...
const revokeUserInvite = async (req, res) => {
  ...
  // VAPT (Lack of Input Validation, CWE-20): strip control/newline characters
  // to prevent audit-log injection, cap length BEFORE escaping so the limit
  // reflects the actual reason text, then HTML-escape so the stored value is
  // inert if ever rendered as HTML (matches escapeHtml() usage on name fields
  // elsewhere in this codebase).
  reason = reason.replace(/[\r\n\t\x00-\x1F\x7F]/g, " ").trim();
  reason = escapeHtml(reason.slice(0, 255));
