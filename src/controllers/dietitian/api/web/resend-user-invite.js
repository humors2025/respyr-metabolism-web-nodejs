 ...
 *  - Actor re-checked against DB.
 *  - Fully parameterized SQL queries.
 *  - Fresh cryptographically-secure invite token.
 *  - Only HMAC-SHA256 token hash stored.
 *  - Stored invitation text is HTML-escaped before entering Resend templates.
 *  - Internal errors suppressed in production.
 *  - Cache-Control: no-store.
 *
 * HIPAA controls:
 *  - Minimum-necessary columns.
 *  - PHI in audit logs is HMAC-SHA256 hashed.
 *  - Every resend action is audited.
 */
...
/**
 * SHA-256 HMAC keyed by SECURITY_PEPPER.
 */
...
/**
 * Format Date as UTC MySQL DATETIME.
 */
...
function getActorEffectivePartnerCode(
  actor
) {
  if (
...
async function writeAuthLogSafe(
  ...
  try {
    ...
    console.error(
      "RESEND_USER_INVITE_AUDIT_FAILED:",
      err?.code ||
        err?.message
    );
  }
...
/**
 * Resolve authenticated actor from JWT and DB.
 */
...
/**
 * Check whether actor is allowed to manage the invitation.
 */
async function actorCanManageInvite(
  ...
  /*
  |--------------------------------------------------------------------------
  | Trainers cannot resend user invitations
  |--------------------------------------------------------------------------
  */
  ...
  */
  ...
  /*
  |--------------------------------------------------------------------------
  | Super admin may manage invites belonging to their active admins
  |--------------------------------------------------------------------------
  */
...
/**
 * Sends invitation email using Resend template.
 *
 * IMPORTANT:
 * User-controlled/stored text must already be HTML-escaped before this helper
 * is called.
 */
  ...
  inviteId <= 0
    ...
    try {
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | 1b. Cross-check optional actor_user_id
      |--------------------------------------------------------------------------
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | 2. Read invitation under DB lock
      |--------------------------------------------------------------------------
      */
...
*/
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | HTML Injection defense for stored invitation data
      |--------------------------------------------------------------------------
      |
      | New invitation endpoints reject malicious markup before storage.
      |
      | However, this endpoint may still read OLD/LEGACY database records
      ...
      | {{7*7}}
      |
      | Therefore all plain-text template variables are escaped again here
      | immediately before they enter the Resend HTML template.
      |--------------------------------------------------------------------------
      */
      ...
      const emailResult =
        await sendResendTemplateEmail(
...

          {
            ...
            |--------------------------------------------------------------------------
            |
            | inviteLink is created server-side using crypto.randomBytes()
            | and a configured base URL. It is not user-provided name data.
            |--------------------------------------------------------------------------
            */
      ...
      */
      ...
      if (
        ...
        console.error(
          "RESEND_USER_INVITE_EMAIL_FAILED:",
          {
            invitation_id:
              inviteId,
      ...
      |--------------------------------------------------------------------------
      |
      | Guarded against a concurrent invitation acceptance.
      |--------------------------------------------------------------------------
      */
      ...
      if (
        ...
        await writeAuthLogSafe(
          req,
          {
            ...
            failureReason:
              "Invite no longer resendable (accepted concurrently)",
          }
        );
