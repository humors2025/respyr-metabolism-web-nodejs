 ...
 *
 * Converted from: the weekly_food_json_suggestions upsert PHP.
 * Platform      : Respyr Dietitian API (api.respyr.ai)
 * Security      : VAPT-hardened, HIPAA-aligned
 *
 * Endpoint : POST /dietitian/api/web/store_weekly_food_json_suggestion
 ...
 *            rollups) for one client into weekly_food_json_suggestions.
 *
 * ──────────────────────────────────────────────────────────────────────────
 ...
 *  - Same business table only: weekly_food_json_suggestions. table_clients is
...
async function writeAuthLogSafe(req, {
  ...
  try {
    ...
    console.error("STORE_WEEKLY_FOOD_JSON_AUDIT_FAILED:", err?.code || err?.message);
  }
}
...
/**
 * POST /dietitian/api/web/store_weekly_food_json_suggestion
 *
 ...
 *   }
 */
const storeWeeklyFoodJsonSuggestion = async (req, res) => {
  ...
  try {
    ...
    const [insertResult] = await connection.execute(
      `
        INSERT INTO weekly_food_json_suggestions
        (
          dietician_id,
          profile_id,
          week_start_date,
    ...
    if (connection) {
      try {
        ...
        console.error(
          "STORE_WEEKLY_FOOD_JSON_ROLLBACK_FAILED:",
    ...
    console.error("STORE_WEEKLY_FOOD_JSON_ERROR:", {
      code: err?.code,
      errno: err?.errno,
...
module.exports = { storeWeeklyFoodJsonSuggestion };
