...
const { ApiError, APP_DEBUG, RESEND_API_KEY } = csi;
...
async function getTrainerByCode(
  conn,
  trainerCode
) {
  const [rows] =
    await conn.execute(
      `SELECT
       ...
       WHERE UPPER(td.dietician_id) =
             UPPER(?)

          OR UPPER(aur.partner_code) =
             UPPER(?)

       ORDER BY
         CASE
           WHEN aur.status = 'active'
           THEN 0
           ELSE 1
         END,
...
async function findExistingLatestInvite(
  conn,
  trainerId,
  trainerCode,
  clientEmail
...
async function hasPendingFreeTrial(
  conn,
  trainerCode,
  clientEmail
) {
...
async function createPlanSubscription(
  ...
  const redeemCode =
    await csi.uniqueRedeemCode(
      conn
    );

  const trialStartedAt =
    csi.istMysqlDateTime(
      new Date()
    );
    ...
    */
    ...
    try {
      ...
      */
      ...
      |
      | Do not silently clean malicious characters.
      |
      | Example:
      |
      ...
      */
      ...
      if (
        !clientMobileResult.ok
      ) {
        throw new ApiError(
          400,
          clientMobileResult.message
        );
      }
      ...
      */
      ...
      const clientMobile =
        clientMobileResult.value;

      /*
      |--------------------------------------------------------------------------
      | Preserve original endpoint's 10-15 digit phone requirement
      |--------------------------------------------------------------------------
      */
      ...
      */
      ...
      const {
        ...
        await csi.resolveActorFromToken(
          conn,
          req
        );
      ...
      /*
      |--------------------------------------------------------------------------
      | actor_user_id is only a cross-check
      |--------------------------------------------------------------------------
      */
      ...
      if (
        ...
        await csi.audit(
          ...
          "client_free_trial_invite_denied",
      ...
      /*
      |--------------------------------------------------------------------------
      | Allowed trainer codes
      |--------------------------------------------------------------------------
      */

      const allowedCodes =
        await csi.allowedCodes(
          conn,
          actor,
          actorEmail
        );
      ...
      if (
        ...
        await csi.audit(
          ...
          "actor not allowed to use this trainer code"
        );

        throw new ApiError(
          403,
          "You are not allowed to invite clients using this trainer code"
        );
      }
      ...
      */
      ...
      if (
        ...
        throw new ApiError(
          403,
          "Trainer/admin account is not active"
        );
      }

      const trainerId =
        csi.clean(
          trainer.dietician_id
        );
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | Pending free-trial check
      |--------------------------------------------------------------------------
      */

      const pendingFreeTrial =
        await hasPendingFreeTrial(
          conn,
          trainerCode,
          clientEmail
        );
      ...
      if (
        ...
        throw new ApiError(
          ...
          "This client already has a pending free trial invite",

          {
            data: {
              ...
              redeem_code:
                pendingFreeTrial.redeem_code,

              created_at:
                formatDbDateTime(
                  pendingFreeTrial.created_at
                ),

              code_expires_at:
                formatDbDateTime(
                  pendingFreeTrial.code_expires_at
                ),
      ...
      */
      ...
      const existingInvite =
        await findExistingLatestInvite(
          conn,
          trainerId,
          trainerCode,
          clientEmail
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | Upsert latest invite
      |--------------------------------------------------------------------------
      */

      sourceInviteId =
        await insertOrUpdateLatestInviteIfSafe(
          conn,
      ...
      */
      ...
      |--------------------------------------------------------------------------
      |
      | client-subscription-action-common.js already handles HTML escaping
      | before values are sent into the Resend HTML template.
      |--------------------------------------------------------------------------
      */
      ...
      */
      ...
      await csi.audit(
        ...
        emailResult.success
          ? "client_free_trial_invite_sent"
          : "client_free_trial_invite_failed",
        ...
        emailResult.success
          ? "Client free trial invite sent"
          : "Client free trial invite failed"
      );
      ...
      */
      ...
      console.error(
        "SEND_TRAINER_CLIENT_INVITE_ERROR:",
        {
          code:
            err?.code,
      ...
      */
      ...
      await csi.audit(
        ...
        "client_free_trial_invite_error",
...
//  *    (trainer_code, client_email).
//  *  - Creates a trainer_client_plan_subscriptions row.
...
//  *  - Secure redeem codes.
//  *  - client_name is validated with the centralized Unicode human-name
//  *    allowlist, blocking HTML/template injection before DB storage or email.
//  *  - Internal error details suppressed in production.
...
//       /*
//       |--------------------------------------------------------------------------
//       | HTML Injection protection - client_name
//       |--------------------------------------------------------------------------
//       |
//       | OLD:
//       |
...
//       |
//       | csi.clean() only trimmed the value, meaning input such as:
//       |
//       | <h1>HTML INJECTION</h1>
//       | <script>alert(1)</script>
//       | {{7*7}}
//       |
//       | could be accepted and stored.
//       |
//       | NEW:
//       |
//       | validateHumanName() applies a strict Unicode human-name allowlist.
//       |--------------------------------------------------------------------------
//       */
...
//         throw new ApiError(
//           400,
//           "Valid client_email is required"
//         );
//       }

//       /*
//       |--------------------------------------------------------------------------
//       | client_mobile validation
//       |--------------------------------------------------------------------------
//       */
...
//       /*
//       |--------------------------------------------------------------------------
//       | Optional actor_user_id cross-check
//       |--------------------------------------------------------------------------
//       */
...
//       |--------------------------------------------------------------------------
//       |
//       | trainerName comes from the database.
//       |
//       | We will additionally HTML-escape this in
//       | client-subscription-action-common.js before it enters the email template.
//       |--------------------------------------------------------------------------
//       */
...
//       /*
//       |--------------------------------------------------------------------------
//       | 5. Duplicate active free trial check
//       |--------------------------------------------------------------------------
//       */
...
//       |--------------------------------------------------------------------------
//       |
//       | clientName has already passed strict human-name validation.
//       |
//       | Output-context HTML encoding will additionally be implemented inside
//       | csi.sendEmail() in client-subscription-action-common.js.
//       |--------------------------------------------------------------------------
//       */
...
//       /*
//       |--------------------------------------------------------------------------
//       | Persist subscription email result
//       |--------------------------------------------------------------------------
//       */
