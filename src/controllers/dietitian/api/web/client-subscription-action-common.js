...
const SECURITY_PEPPER =
  ...
  "CHANGE_THIS_LONG_RANDOM_PEPPER";
...
async function getLegacyInviteForUpdate(
  db,
  body
) {
  const inviteId =
...
function subscriptionIsAccepted(
  ...
  if (
    !phpEmpty(
      sub.accepted_profile_id
    ) ||
    !phpEmpty(
      sub.redeemed_profile_id
    )
  ) {
    return true;
  }
...
function legacyInviteIsAccepted(
  invite
) {
  if (!invite) {
    return false;
...
async function uniqueRedeemCode(
  db
) {
  for (
    let i = 0;
  ...
  throw new ApiError(
    500,
    "Could not generate unique redeem code"
  );
}
...
async function createSubscriptionFromLegacyInvite(
  ...
  const planPriceLabel =
    clean(
      legacyInvite.plan_price_label
    ) !== ""
      ? legacyInvite.plan_price_label
      : "Free";

  const redeemCode =
    await uniqueRedeemCode(
      db
    );
  ...
  const [result] =
    await db.execute(
      ...
      [
        ...
        legacyInvite.trainer_id ??
          null,

        legacyInvite.trainer_code ??
          null,

        legacyInvite.client_name ??
          null,

        legacyInvite.client_mobile ??
          null,

        email(
          legacyInvite.client_email
        ),
        ...
        legacyInvite.resend_email_id ??
          null,

        legacyInvite.accepted_profile_id ??
          null,

        legacyInvite.accepted_at ??
          null,

        legacyInvite.error_message ??
          null,
...
async function sendEmail(sub) {
  ...
  */
  ...
  const safeTrainerCode =
    escapeHtml(
      sub.trainer_code ??
      ""
    );

  const safeRedeemCode =
    escapeHtml(
      sub.redeem_code ??
      ""
    );

  const safeCodeExpiresAt =
    escapeHtml(
      sub.code_expires_at ??
      ""
    );
  ...
  const safePlanPriceLabel =
    escapeHtml(
      sub.plan_price_label ??
      ""
    );
  ...
  */
  ...
  const safeExtraVariables =
    {};

  if (
    sub.extra_variables &&
    typeof sub.extra_variables ===
      "object" &&
    !Array.isArray(
      sub.extra_variables
    )
...
async function audit(
  ...
  try {
    ...
    console.error(
      "CLIENT_SUBSCRIPTION_AUDIT_FAILED:",
      err?.code ||
      err?.message
    );
  }
