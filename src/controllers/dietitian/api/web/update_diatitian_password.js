 ...
 *
 * Endpoint : POST /auth/update_diatitian_password
 * Access   : PUBLIC, but requires a valid single-use reset token
 *
 * Purpose:
 ...
 * - Strict reset-token format validation.
 * - Single-use reset token issued only after successful OTP verification.
 * - Strong password policy.
 * - bcrypt password hashing.
 * - Current password cannot be reused.
 * - All active refresh-token sessions are revoked after password change.
 * - Parameterized SQL.
 * - Audit logging without clear-text identifiers.
 */
...
const otpStore = require(
  "../../../../services/dietitianOtpStore"
);
...
|--------------------------------------------------------------------------
|
| dietitianOtpStore.js creates the token using:
|
| crypto.randomBytes(32).toString("hex")
|
| Therefore a legitimate reset token is exactly:
|
...
async function writeAuthLogSafe(
  ...
  try {
    ...
    /*
    |--------------------------------------------------------------------------
    | Use active transaction connection when available.
    |--------------------------------------------------------------------------
    */
...
exports.updateDietitianPassword =
  async (
    req,
    res
  ) => {
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Reset token must be a string and exactly 64 hex characters.
    |
    | This prevents arbitrary malformed input from reaching the OTP-token store.
    |--------------------------------------------------------------------------
    */

    if (
      typeof body.reset_token !==
      "string"
    ) {
      return res
        .status(400)
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Passwords are secrets, so HTML/special characters must NOT be stripped.
    |
    | We only enforce type and password policy.
    |--------------------------------------------------------------------------
    */
    ...
    */
    ...
    try {
      /*
      |--------------------------------------------------------------------------
      | 1. Consume single-use reset token
      |--------------------------------------------------------------------------
      |
      | This token is created only after successful OTP verification.
      |--------------------------------------------------------------------------
      */
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | Account vanished between OTP verification and reset
      |--------------------------------------------------------------------------
      */
      ...
      if (
        ...
        await writeAuthLogSafe(
          ...
          req,
          {
            ...
            failureReason:
              "Account not found at reset time",
          }
        );
      ...
      |--------------------------------------------------------------------------
      |
      | Database values are also treated as untrusted defense-in-depth.
      |--------------------------------------------------------------------------
      */
      ...
      if (
        ...
        await writeAuthLogSafe(
          ...
          req,
          {
            ...
            failureReason:
              "Stored account email validation failed",
          }
        );
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | 3. Reject current password reuse
      |--------------------------------------------------------------------------
      |
      | PHP bcrypt hashes beginning with $2y$ are converted to $2b$ for Node.
      |--------------------------------------------------------------------------
      */
      ...
      */
      ...
      /*
      |--------------------------------------------------------------------------
      | Update must affect exactly one account
      |--------------------------------------------------------------------------
      */
      ...
      |--------------------------------------------------------------------------
      |
      | Credential changes should invalidate existing sessions.
      |--------------------------------------------------------------------------
      */
