...
//
// Secrets (DB creds, JWT secret, Resend key, service API keys) now live in
// AWS Secrets Manager (respyr/prod/backend), not Lambda env vars. Many
// controllers capture these into module-load-time constants, so process.env
// MUST be hydrated BEFORE ./src/index is required. That is why the app is
// required lazily inside buildHandler() instead of at the top of this file.
//
// The built handler is cached per container: hydration + require happen once
// per cold start; warm invocations reuse handlerPromise.
// ---------------------------------------------------------------------------
...
async function buildHandler() {
  ...
  // Safe to require now — process.env carries the secret values.
...
module.exports.handler = async (event, context) => {
  if (!handlerPromise) {
    handlerPromise = buildHandler().catch((err) => {
      ...
      // container being permanently stuck with a rejected promise.
      handlerPromise = null;
      throw err;
    });
  }
